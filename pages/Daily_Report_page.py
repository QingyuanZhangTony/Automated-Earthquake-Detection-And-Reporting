...
# Application Title
title_help_text = (
    "This page guides you through the complete process of generating a daily earthquake report manually. "
    "Follow the steps below to download stream data, process the signals, detect seismic phases, "
    "match events with the earthquake catalog, and finally generate and send the report via email."
)

st.title("Manual Daily Report Generation",help=title_help_text)


reset_container = st.container()
with reset_container:
    if st.button('Reset Page', key='reset_button', help="Click this button to reset the session. "
                                                           "This will clear all current session data and reinitialize the default settings."):
...
with catalog_settings_container:
    catalog_help_text = "This step sequentially requests the earthquake catalog from the specified providers, ensuring that if one provider is unavailable, the next in line is automatically queried."
...
with station_settings_container:
    station_help_text = "This step involves the retrieval of seismic stream data from the specified station. The data acquisition process includes accessing the station's network, selecting the appropriate channels, and downloading the waveform data for the designated time period."
...
with process_stream_container:
    process_help_text = "This step conducts comprehensive processing of the seismic data stream, including tasks such as detrending, filtering, and noise reduction. These procedures are essential for refining the raw seismic signals, enhancing the quality of the data for subsequent analysis, and ensuring accurate phase detection and event matching."
...
with detect_phases_container:
    detect_phases_help_text = "This step involves the identification of P and S seismic phases within the acquired stream data, utilizing the DeepDenoiser model. The detected phases are assigned confidence scores, which indicate the model's certainty in the accuracy of each detection."
...
with match_events_container:
    match_events_help_text = "This step matches detected P/S waves with the earthquakes from the catalog, using confidence scores and time tolerance thresholds. The matching process ensures that only high-confidence detections within a specified time window around the predicted earthquake arrival times are considered, improving the accuracy of event associations."
...
with generate_report_container:
    report_help_text = "This step generates a detailed report for successfully matched earthquakes, including visualizations, event summaries, and statistical analysis, along with the complete catalog information for the day."
